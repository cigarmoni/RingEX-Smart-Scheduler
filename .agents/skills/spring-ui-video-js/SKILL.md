---
name: spring-ui-video-js
description: >
  *REQUIRED READING!* READ THIS INSTEAD OF THE video-js SKILL. THAT SKILL IS NOW *DEPRECATED*.
  Create agency-quality animated videos programmatically using React, Framer Motion, GSAP, and Three.js with Spring UI design system. Use for marketing videos, motion graphics, animated presentations, or broadcast-quality video content that runs in the browser. Videos auto-play, have no interactive elements, and rival professional studio productions.
---
...
1. Call `subagent()` with `specialization="DESIGN"`, passing:
   - The user's request as the task, unchanged, prefixed with: "You are the subagent executing this task, see the skill: .agents/skills/spring-ui-video-js/SKILL.md\n\n"
   - `.agents/skills/spring-ui-video-js/SKILL.md` and `.agents/skills/spring-ui-video-js/references/finalize_playback.md` as `relevantFiles` -- this auto-opens the skill for the subagent so it can read it directly
...
const result = await subagent({
    task: `You are the subagent executing this task, see the skill: .agents/skills/spring-ui-video-js/SKILL.md\n\n${userRequest}`,
    specialization: "DESIGN",
    relevantFiles: [".agents/skills/spring-ui-video-js/SKILL.md", ".agents/skills/spring-ui-video-js/references/finalize_playback.md"]
});
console.log(result);
```
...
<technical_reference>
**Framework:** React + Tailwind CSS (v3) + Spring UI (`@ringcentral/spring-ui` with `ThemeProvider`)
...
**Icons:** @ringcentral/spring-icon (use sparingly), or inline SVGs
...
After creating a video-js artifact with `createArtifact()`, immediately convert it to Spring UI:

```bash
bash .agents/skills/spring-ui-video-js/scripts/convert-to-spring-ui.sh artifacts/<slug>
```

This replaces the default scaffold with Tailwind v3 + Spring UI (ThemeProvider, spring-theme). The video-specific infrastructure (useVideoPlayer hook, animation presets, scene components) is preserved.
</build_setup>
...
- The workflow runs `npm run dev` via Vite
...
- `task`: "Finalize video polish -- verify all content fits within the 16:9 frame without cutoff, remove any responsive breakpoints, and confirm the loop is clean. Follow the instructions in .agents/skills/spring-ui-video-js/references/finalize_playback.md exactly."
- `relevant_files`: [".agents/skills/spring-ui-video-js/references/finalize_playback.md"]
