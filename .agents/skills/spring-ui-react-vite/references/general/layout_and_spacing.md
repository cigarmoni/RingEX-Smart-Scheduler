...
When using `justify-between`, `justify-around`, or `justify-evenly`, you **must** also apply `gap` classes to handle scenarios where there isn't enough space:
...
When using Spring UI `<Button>`, never control padding or height explicitly—use the `size` and `variant` props:

```tsx
// Bad - controlling button sizing manually
<Button className="px-8 py-3 h-12">Submit</Button>

// Good - use the size prop
<Button size="large" variant="contained" color="primary">Submit</Button>
```

Available sizes: `xlarge`, `large`, `medium` (default), `small`, `xsmall`
Available variants: `contained` (default), `outlined`, `text`, `inverted`
Available colors: `primary` (default), `secondary`, `neutral`, `danger`, `warning`, `success`
...
- Spring spacing tokens: `p-1` (0.25rem), `p-2` (0.5rem), etc. — mapped to `--sui-spacing-*`
