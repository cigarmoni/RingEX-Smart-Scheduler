---
name: spring-ui-data-visualization
description: >
  BOTH THIS SKILL AND THE REGULAR DATA VISUALIZATION SKILL ARE DEPRICATED USE THE SPRING-UI-REACT-VITE SKILL INSTEAD.
---

# Data Visualization Skill (Spring UI)

This skill helps you create interactive data visualization apps with charts, data tables, and CSV handling using the Spring UI design system.
...
This skill uses the **react-vite scaffold** converted to **Spring UI** (Tailwind v3 + RingCentral Spring UI components), with backend conventions from the **`pnpm-workspace` skill**:

1. Create the artifact (`createArtifact()` with type `data-visualization`)
2. **Run the Spring UI conversion script** immediately after creation:
   ```bash
   bash .agents/skills/spring-ui-react-vite/scripts/convert-to-spring-ui.sh artifacts/<slug>
   ```
3. Install data-viz packages and patch CSS (chart colors + print styles)
4. Follow the contract-first backend flow from the `pnpm-workspace` skill (`references/openapi.md`, `references/server.md`, `references/db.md`)
5. Launch a GENERAL subagent (async) for the frontend, adding `.agents/skills/spring-ui-react-vite/SKILL.md` to `relevantFiles` so it reads the Spring UI component docs
6. Implement API routes in the shared `artifacts/api-server/`

See `references/common-bootstrap.md` for the full step-by-step workflow.

**IMPORTANT:** The subagent is a bottleneck launch it as quickly as possible.

**IMPORTANT:** Data visualization artifacts use the react-vite scaffold under the hood. The Spring UI conversion script from `spring-ui-react-vite` handles all design system setup (Tailwind v3, Spring UI components, ThemeProvider). No additional conversion is needed.

**IMPORTANT:** Data visualization artifacts must use the GENERAL subagent workflow for frontend implementation. The reference files contain critical layout and styling specifications that only the subagent can consume. When launching a subagent, include `.agents/skills/spring-ui-react-vite/SKILL.md` in `relevantFiles` so it knows how to use Spring UI components.
...
- `references/common-css-overrides.md` — CSS patches, fonts, shadows, chart colors, print styles
...
## Spring UI Component Reference

For Spring UI component usage (buttons, cards, inputs, dialogs, etc.), read `.agents/skills/spring-ui-react-vite/SKILL.md`. This is the authoritative reference for all Spring UI components and patterns.

## Content Rules (MANDATORY for all UI text)
All visible text in any dashboard, report, or explorer MUST follow the RingCentral Content Companion rules (`.agents/skills/rc-content-companion/SKILL.md`). This applies to every label, button, heading, description, error message, empty state, tooltip, chart title, axis label, and placeholder.
### Quick Reference
