...
name: spring-ui-expo
description: >
  *REQUIRED READING!* READ THIS INSTEAD OF THE expo SKILL. THAT SKILL IS NOW *DEPRECATED*.
...
## Spring UI — Token-Only Conversion

Spring UI's React components (`@ringcentral/spring-ui`) are **web-only** and cannot run in React Native. However, the Spring UI *design tokens* — colors, typography, spacing, and border radii — can be mapped directly to React Native's `StyleSheet` system. This gives Expo apps the same visual identity as Spring UI web apps without changing the tech stack.

### Conversion depth: Token-only (style layer swap)

- **No npm packages are added or removed.** The tokens are pure TypeScript constants.
- The conversion script copies two files into `constants/`:
  - `theme.ts` — full Spring UI token set (light + dark palettes, typography scale, spacing, radii)
  - `colors.ts` — updated to import from `theme.ts` (replaces the scaffold default)

### Running the conversion

After every `createArtifact()` call for an Expo artifact, immediately run:

```bash
bash .agents/skills/spring-ui-expo/scripts/convert-to-spring-ui.sh artifacts/<name>
```

### Using the tokens

Token values are extracted directly from `@ringcentral/spring-theme@1.8.0` (`themes/light.js` and `themes/dark.js`). Light and dark palettes are separate objects — not shared — because primary, danger, success, and warning colors differ between themes.

Import from `constants/theme.ts` and use in `StyleSheet.create()`:
...
const styles = StyleSheet.create({
  ...
  primaryButton: {
    backgroundColor: theme.colors.primary.b,
...
Typography presets include `fontFamily` already — each preset maps its weight to the correct loaded font name (e.g. `Inter_600SemiBold`). Just spread and go:
...
const styles = StyleSheet.create({
  ...
  label: { ...typography.labelSemiBold },
});
```

The Expo scaffold loads `Inter_400Regular`, `Inter_500Medium`, `Inter_600SemiBold`, and `Inter_700Bold` in `_layout.tsx`. The typography presets reference these exact names via `fontFamily`. If you swap to a different font package, update the `fonts` object at the top of `theme.ts` to match your loaded font names.

**Important React Native font rule:** In React Native, you cannot use `fontFamily: 'Inter'` with a separate `fontWeight` — you must use the full loaded name like `Inter_700Bold`. The typography presets handle this for you.

### Token mapping to Spring UI web equivalents

| Spring UI (Tailwind) | Expo (theme.ts) |
|---|---|
| `bg-neutral-base` | `theme.colors.neutral.base` |
| `bg-neutral-b5` | `theme.colors.neutral.b5` |
| `text-neutral-b0` | `theme.colors.neutral.b0` |
| `text-neutral-b2` | `theme.colors.neutral.b2` |
| `bg-primary-b` | `theme.colors.primary.b` |
| `text-danger` | `theme.colors.danger.base` |
| `text-success` | `theme.colors.success.base` |
| `text-warning` | `theme.colors.warning.base` |
| `border-neutral-b4` | `theme.colors.neutral.b4` |
| typography: main-text | `typography.mainText` (14px/20) |
| typography: subtitle | `typography.subtitle` (15px/20) |
| typography: title | `typography.title` (17px/25) |
| typography: display3 | `typography.display3` (20px/28) |
| typography: headline | `typography.headline` (28px/36) |
| `rounded-md` | `borderRadius.md` (14) |
| `rounded-sm` | `borderRadius.sm` (10) |
| `spacing-4` (1rem) | `spacing[4]` (16) |
| `spacing-6` (1.5rem) | `spacing[6]` (24) |

### Dark mode

Both `lightTheme` and `darkTheme` are exported. Use React Native's `useColorScheme()` to switch between them — the same semantic token names work for both themes, just like Spring UI's `ThemeProvider` on web.
