---
name: spring-ui-slides
description: >
  *REQUIRED READING!* READ THIS INSTEAD OF THE slides SKILL. THAT SKILL IS NOW *DEPRECATED*.
  This is the RingCentral Spring UI slides skill. It replaces shadcn + Tailwind v4 with
  Spring UI + Tailwind v3. It contains all slide component documentation, manifest conventions,
  visual editing compatibility, and design guidance for creating presentations.
  Always use this when the user is creating any slides artifact.
---

# Slides -- Presentation Decks in Code (Spring UI)

<context>
A slides artifact is a React + Tailwind CSS (v3) + Spring UI application that functions as a slide deck. Each slide is a separate React component file in `src/pages/slides/`, rendered at a unique `/slideN` URL route (e.g., `/slide1`, `/slide2`). This React app runs inside a workspace app preview, where the preview wraps it in a custom slide viewer / editor UI. That UI provides a thumbnail sidebar for navigation, PPTX export, and visual editing controls that let the user reorder slides, add or delete slides, and edit visual properties like colors and text directly from the Replit interface.
...
- A `GENERAL` subagent (with `.agents/skills/spring-ui-slides/SKILL.md` in `relevantFiles`) can create SVG animations for persistent or looping motion effects within a slide.
...
1. Create the slides artifact with `createArtifact({ artifactType: "slides", ... })`.
2. Run the Spring UI conversion script immediately after creation:
   ```
   bash .agents/skills/spring-ui-slides/scripts/convert-to-spring-ui.sh artifacts/<slug>
   ```
3. Establish creative direction (palette, fonts, aesthetic). Write it out before any code.
4. Plan assets -- inventory user-attached files, plan supplemental images.
5. Set up `index.html` (fonts) and `index.css` (CSS variables).
6. Build slides in `src/pages/slides/`. Title slide first to lock in the visual system.
7. Update `src/data/slides-manifest.json` manifest for each slide.
8. Run `pnpm run --filter @workspace/<slug> validate-slides` and fix any issues.
9. Run quality checks.
10. Present the artifact.
