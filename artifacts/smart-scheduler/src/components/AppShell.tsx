...
import { Button, IconButton, Popover, Menu, MenuItem } from "@ringcentral/spring-ui";
import { AvaDrawer } from "./AvaDrawer";
import { useAva } from "../contexts/AvaContext";
import { FlowsLauncher } from "./FlowsLauncher";
...
export const AppShell = ({ activeNav, children, onNavigate }: AppShellProps): JSX.Element => {
  ...
  const handleNavClick = (item: typeof primarySidebarItems[number], e: React.MouseEvent<HTMLButtonElement>) => {
    if (item.isMore) {
      setMoreMenuAnchor(moreMenuAnchor ? null : e.currentTarget);
    } else if (item.href) {
        ...
        <div className="flex h-full w-full flex-col bg-white">
          <header className="flex flex-wrap items-center gap-sui-2 border-b border-[#dddfe580] bg-[#0040dd] px-sui-3 py-sui-2 sm:gap-sui-4 sm:px-sui-4 sm:py-2.5">
            <div className="flex min-w-0 items-center gap-sui-2 sm:gap-sui-3">
              ...
              <h1 className="truncate text-display-2 text-white">
                RingCentral, Inc.
              </h1>
            </div>
            <div className="order-3 flex w-full flex-1 items-center justify-center gap-1.5 md:order-none md:w-auto">
              <div className="hidden items-center gap-sui-1 md:flex">
                ...
                <Button variant="text" className="h-9 w-9 rounded-full p-0 hover:bg-white/10 min-w-0" aria-label="Next">
              ...
              <div className="flex w-full max-w-[468px] flex-1 items-center px-1.5">
                ...
                >
                  ...
                  <span className="flex-1 text-main-text text-white/60">
                    Ask Ava or Search
                  </span>
                </button>
              </div>
                ...
                <div className="w-max rounded-[10px] border border-solid border-[#00000033] bg-white shadow-sui-md">
                  <div className="p-sui-2">
                    <div className="flex flex-col bg-white">
                      ...
                      <div className="my-2 h-[1px] w-full bg-[#0000001a]" />
